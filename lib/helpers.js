...
function getConfig() {
  ...
  try {
    ...
    // no file yet
    fs.writeFileSync(__dirname + '/../store.json', JSON.stringify(defaultConfig));
  }

  try {
    var configContent = fs.readFileSync(__dirname + '/../store.json');
    //If for some reason the file got corrupted and it's now empty'
    if (_.isUndefined(configContent) || _.isEmpty(configContent.toString())) {
